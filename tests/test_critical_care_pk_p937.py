...
def test_cl_effective_matches_reference_times_multiplier():
    ...
    result = simulate_critical_care_pk(
        "Drug_A", dose_mg=500.0, icu_state="sepsis_early", cl_ref_L_per_h=cl_ref
    )
...
def test_sepsis_late_aki_auc_greater_than_normal():
    ...
    aki = simulate_critical_care_pk(
        "Drug_A", dose_mg=500.0, icu_state="sepsis_late_aki", t_end_h=96.0
    )
    normal = simulate_critical_care_pk(
        "Drug_A", dose_mg=500.0, icu_state="normal_icu", t_end_h=96.0
    )
...
def test_iv_infusion_cmax_less_than_bolus():
    ...
    bolus = simulate_critical_care_pk("Drug_A", dose_mg=500.0, route="iv_bolus")
    infusion = simulate_critical_care_pk(
        "Drug_A", dose_mg=500.0, route="iv_infusion", infusion_duration_h=24.0, t_end_h=48.0
    )
...
def test_t_half_formula():
    ...
    result = simulate_critical_care_pk(
        "Drug_A", dose_mg=500.0, icu_state="normal_icu", cl_ref_L_per_h=cl_ref, vd_ref_L=vd_ref
    )
...
def test_times_length_correct():
    result = simulate_critical_care_pk("Drug_A", dose_mg=500.0, t_end_h=10.0, dt_h=1.0)
...
def test_route_preserved():
    result = simulate_critical_care_pk(
        "Drug_A", dose_mg=500.0, route="iv_infusion", infusion_duration_h=8.0, t_end_h=24.0
    )
