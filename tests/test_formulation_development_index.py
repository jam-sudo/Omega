...
def test_subscores_sum_to_fdi():
    ...
    total = (
        r.solubility_score
        + r.permeability_score
        + r.chemical_stability_score
        + r.physical_stability_score
        + r.dose_number_score
    )
...
def test_michael_acceptor_parenteral():
    r = calculate_fdi(
        "Drug", solubility_mg_mL=10.0, logP=2.0, dose_mg=100.0, has_michael_acceptor=True
    )
...
def test_invalid_melting_point_raises():
    with pytest.raises(ValueError, match="melting_point_C"):
        calculate_fdi("X", solubility_mg_mL=10.0, logP=2.0, dose_mg=100.0, melting_point_C=10.0)
