...
def test_short_interval_higher_accumulation():
    ...
    short = simulate_accumulation("Drug", dose_mg=100.0, dosing_interval_h=4.0,
                                   cl_L_per_h=10.0, vd_L=50.0)
    long = simulate_accumulation("Drug", dose_mg=100.0, dosing_interval_h=24.0,
                                  cl_L_per_h=10.0, vd_L=50.0)
...
def test_accumulation_ratio_formula():
    ...
    dose_mg = 100.0
    vd_L = 50.0
    result = simulate_accumulation("Drug", dose_mg=dose_mg, vd_L=vd_L,
                                    dosing_interval_h=12.0, cl_L_per_h=5.0, route="iv")
    c0 = dose_mg / vd_L
...
def test_long_t_half_high_accumulation():
    ...
    result = simulate_accumulation("LongHalf", dose_mg=100.0, cl_L_per_h=0.5,
                                    vd_L=50.0, dosing_interval_h=12.0)
...
def test_oral_route_works():
    result = simulate_accumulation("Drug", dose_mg=100.0, route="oral",
                                    ka_per_h=1.5, bioavailability=0.8)
    assert result.css_max_theoretical > 0
