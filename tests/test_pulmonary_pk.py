...
def test_plasma_peaks_after_lung(default_result: PulmonaryPKResult) -> None:
    lung_peak_idx = default_result.c_lung_tissue_mg_L.index(
        max(default_result.c_lung_tissue_mg_L)
    )
    plasma_peak_idx = default_result.c_plasma_mg_L.index(
        max(default_result.c_plasma_mg_L)
    )
