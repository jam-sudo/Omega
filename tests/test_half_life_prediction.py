...
def test_acid_longer_than_neutral():
    r_neutral = predict_half_life(
        "A", mw_Da=350.0, logP=2.0, psa_A2=80.0, ionization_type="neutral"
    )
    r_acid = predict_half_life("B", mw_Da=350.0, logP=2.0, psa_A2=80.0, ionization_type="acid")
    assert r_acid.t_half_predicted_h > r_neutral.t_half_predicted_h


def test_ester_reduces_half_life():
    r_no_ester = predict_half_life("A", mw_Da=350.0, logP=2.0, psa_A2=80.0, has_ester=False)
    r_ester = predict_half_life("B", mw_Da=350.0, logP=2.0, psa_A2=80.0, has_ester=True)
...
def test_long_classification():
    r = predict_half_life(
        "Big", mw_Da=25000.0, logP=6.0, psa_A2=20.0, ionization_type="neutral", n_rotatable_bonds=0
    )
...
def test_invalid_rot_bonds_raises():
    with pytest.raises(ValueError, match="n_rotatable_bonds"):
        predict_half_life("X", mw_Da=350.0, logP=2.0, psa_A2=80.0, n_rotatable_bonds=-1)


def test_invalid_ionization_type_raises():
    with pytest.raises(ValueError, match="ionization_type"):
        predict_half_life("X", mw_Da=350.0, logP=2.0, psa_A2=80.0, ionization_type="zwitterion")
