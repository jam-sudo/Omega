...
def test_labile_class_for_high_k():
    ...
    r = predict_matrix_stability(
        "Drug", "liver_s9",
        has_ester=True, has_hydroxamate=True,
        base_k_per_h=0.5,
    )
...
def test_labile_recommendation_mentions_additives():
    ...
    assert r.stability_class == "labile"
    assert "stabilising" in r.bioanalytical_recommendation.lower() or "stabiliz" in r.bioanalytical_recommendation.lower() or "additive" in r.bioanalytical_recommendation.lower()
