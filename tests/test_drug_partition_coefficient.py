...
def test_all_kp_values_positive():
    result = predict_partition_coefficients(
        "DrugA", logp=1.5, pka=5.0, ionization_type="neutral", fup=0.4
    )
...
def test_kp_fat_highest_for_high_logp():
    result = predict_partition_coefficients(
        "Lipophilic", logp=5.0, pka=7.4, ionization_type="neutral", fup=0.5
    )
...
def test_kp_brain_gt_muscle_for_basic_lipophilic():
    result = predict_partition_coefficients(
        "BasicDrug", logp=3.0, pka=9.0, ionization_type="base", fup=0.3
    )
...
def test_basic_drug_higher_kp_brain_than_neutral():
    r_neutral = predict_partition_coefficients(
        "Neutral", logp=2.5, pka=7.4, ionization_type="neutral", fup=0.4
    )
    r_base = predict_partition_coefficients(
        "Base", logp=2.5, pka=9.0, ionization_type="base", fup=0.4
    )
...
def test_result_stores_drug_name():
    result = predict_partition_coefficients(
        "Aspirin", logp=1.2, pka=3.5, ionization_type="acid", fup=0.1
    )
