...
def test_dataclass_fields():
    result = _default_result()
    for field in (
        "victim_drug", "perpetrator_drug", "baseline_fup", "displaced_fup",
        "fup_ratio", "cl_ratio", "auc_ratio", "is_clinically_significant",
        "binding_site", "mechanism", "recommendation",
    ):
...
def test_raises_value_error_baseline_fup_zero():
    with pytest.raises(ValueError):
        assess_protein_displacement("w", "i", baseline_fup=0.0, ki_app_uM=50.0, competitor_conc_uM=10.0)


def test_raises_value_error_baseline_fup_negative():
    with pytest.raises(ValueError):
        assess_protein_displacement("w", "i", baseline_fup=-0.1, ki_app_uM=50.0, competitor_conc_uM=10.0)
...
def test_raises_value_error_ki_app_zero():
    with pytest.raises(ValueError):
        assess_protein_displacement("w", "i", baseline_fup=0.1, ki_app_uM=0.0, competitor_conc_uM=10.0)


def test_raises_value_error_ki_app_negative():
    with pytest.raises(ValueError):
        assess_protein_displacement("w", "i", baseline_fup=0.1, ki_app_uM=-5.0, competitor_conc_uM=10.0)
...
def test_raises_value_error_negative_conc():
    with pytest.raises(ValueError):
        assess_protein_displacement("w", "i", baseline_fup=0.1, ki_app_uM=50.0, competitor_conc_uM=-1.0)
...
def test_binding_site_custom():
    result = assess_protein_displacement(
        "w", "i", 0.05, 50.0, 10.0, binding_site="albumin_site_II"
    )
