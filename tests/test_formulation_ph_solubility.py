...
def test_neutral_constant_solubility():
    result = predict_ph_solubility("NeutralDrug", s0_mg_mL=0.5, pka=7.0, ionization_type="neutral")
    sol = list(result.solubility_mg_mL)
...
def test_neutral_max_equals_s0():
    result = predict_ph_solubility("NeutralDrug", s0_mg_mL=2.0, pka=7.0, ionization_type="neutral")
...
def test_invalid_ph_range_order():
    with pytest.raises(ValueError, match="ph_range_start"):
        predict_ph_solubility(
            "X", s0_mg_mL=1.0, pka=5.0, ionization_type="acid", ph_range_start=8.0, ph_range_end=2.0
        )
...
def test_invalid_ph_range_equal():
    with pytest.raises(ValueError, match="ph_range_start"):
        predict_ph_solubility(
            "X", s0_mg_mL=1.0, pka=5.0, ionization_type="acid", ph_range_start=5.0, ph_range_end=5.0
        )
