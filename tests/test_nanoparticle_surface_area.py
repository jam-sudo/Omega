...
class TestCoating:
    ...
    def test_coating_reduces_effective_drug_loading(self):
        r_no_coat = _default(
            particle_diameter_nm=200.0, coating_thickness_nm=0.0, drug_loading_pct=20.0
        )
        r_coated = _default(
            particle_diameter_nm=200.0, coating_thickness_nm=50.0, drug_loading_pct=20.0
        )
...
class TestShapeFactors:
    ...
    def test_cube_intermediate_surface_area(self):
        ...
        r_rod = _default(particle_shape="rod")
        assert (
            r_sphere.surface_area_m2_per_g
            < r_cube.surface_area_m2_per_g
            < r_rod.surface_area_m2_per_g
        )
