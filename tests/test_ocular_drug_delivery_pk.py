...
def test_a_implant_monotonically_decreases():
    ...
    for i in range(1, len(res.a_implant_mg)):
        assert res.a_implant_mg[i] <= res.a_implant_mg[i - 1], f"a_implant increased at step {i}"
...
def test_contact_lens_drug_load_depleted_faster():
    ...
    res = simulate_ocular_drug_delivery("Drug", 1.0, device_type="contact_lens", t_end_h=720.0)
