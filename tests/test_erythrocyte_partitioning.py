...
def test_fu_blood_le_fu_plasma_when_kp_gt1():
    ...
    result = predict_rbc_partitioning(
        "Drug", logP=5.0, ionization_type="base", pka=10.0, fu_plasma=0.5
    )
...
def test_high_kp_rbc_gives_high_risk():
    ...
    result = predict_rbc_partitioning(
        "ChloroquineAnalog", logP=4.0, ionization_type="base", pka=10.0
    )
...
def test_clinical_implication_high_rbc_contains_reservoir():
    ...
    assert (
        "RBC" in result.clinical_implication
        or "reservoir" in result.clinical_implication
        or "accumulation" in result.clinical_implication
        or "blood" in result.clinical_implication.lower()
    )
...
def test_hematocrit_effect_high_kp():
    ...
    low_hct = predict_rbc_partitioning(
        "Drug", logP=4.0, ionization_type="base", pka=10.0, hematocrit=0.30
    )
    high_hct = predict_rbc_partitioning(
        "Drug", logP=4.0, ionization_type="base", pka=10.0, hematocrit=0.60
    )
...
def test_amphoteric_high_pka_uses_base_model():
    ...
    amphoteric = predict_rbc_partitioning(
        "Amphoteric", logP=3.0, ionization_type="amphoteric", pka=9.0
    )
