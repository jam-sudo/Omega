...
def test_higher_temp_higher_rate():
    assert _run(storage_temp_C=37.0).aggregation_rate_per_h > _run(storage_temp_C=4.0).aggregation_rate_per_h


def test_near_pi_higher_rate():
    assert _run(formulation_ph=7.0, pi=7.0).aggregation_rate_per_h > _run(formulation_ph=5.0, pi=7.0).aggregation_rate_per_h


def test_glycosylation_reduces_rate():
    assert _run(has_glycosylation=True).aggregation_rate_per_h < _run(has_glycosylation=False).aggregation_rate_per_h


def test_disulfide_reduces_rate():
    assert _run(has_disulfide=True).aggregation_rate_per_h < _run(has_disulfide=False).aggregation_rate_per_h


def test_higher_hydrophobicity_higher_rate():
    assert _run(hydrophobicity_score=9.0).aggregation_rate_per_h > _run(hydrophobicity_score=1.0).aggregation_rate_per_h


def test_higher_conc_higher_rate():
    assert _run(protein_conc_mg_mL=100.0).aggregation_rate_per_h > _run(protein_conc_mg_mL=1.0).aggregation_rate_per_h
...
def test_mechanism_valid_values():
    assert _run().primary_aggregation_mechanism in {
        "hydrophobic", "electrostatic", "stress_induced", "chemical"
    }
