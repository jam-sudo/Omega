...
def test_MCC_harder_than_starch_same_force():
    assert _pred(excipient="MCC", force=10.0).hardness_N > _pred(excipient="starch", force=10.0).hardness_N
...
def test_optimize_keys_present():
    result = optimize_compression_force("D", "MCC", 10.0, target_hardness_N=60.0)
    for key in ("min_force_kN", "expected_hardness_N", "expected_friability_pct", "quality_status", "achievable"):
