...
def test_return_type():
    r = predict_cord_blood_level("Morphine", mw_Da=285.0, logP=0.9,
...
def test_fetal_concentration_non_negative():
    r = predict_cord_blood_level("DrugA", mw_Da=300.0, logP=2.0,
                                  ionization_type="neutral")
    assert r.c_fetal_blood_mg_L >= 0.0


def test_small_molecule_high_transfer():
    r = predict_cord_blood_level("SmallDrug", mw_Da=200.0, logP=1.5,
                                  ionization_type="neutral",
                                  fu_maternal=0.9,
                                  c_maternal_mg_L=1.0)
...
def test_biologic_low_transfer():
    r = predict_cord_blood_level("Antibody", mw_Da=150000.0, logP=-2.0,
                                  ionization_type="neutral",
                                  c_maternal_mg_L=1.0)
...
def test_um_ratio_in_range():
    r = predict_cord_blood_level("Drug", mw_Da=300.0, logP=2.0,
                                  ionization_type="neutral")
    assert 0.0 <= r.umbilical_to_maternal_ratio <= 2.0


def test_base_drug_higher_ratio_than_acid():
    r_base = predict_cord_blood_level("Base", mw_Da=300.0, logP=2.0,
                                       ionization_type="base",
                                       fu_maternal=0.3, c_maternal_mg_L=1.0)
    r_acid = predict_cord_blood_level("Acid", mw_Da=300.0, logP=2.0,
...
def test_exposure_class_high_for_small_lipophilic():
    r = predict_cord_blood_level("Drug", mw_Da=200.0, logP=1.5,
                                  ionization_type="neutral",
                                  fu_maternal=0.9,
...
def test_exposure_class_minimal_for_biologic():
    r = predict_cord_blood_level("mAb", mw_Da=150000.0, logP=-2.0,
                                  ionization_type="neutral")
...
def test_linearity_fetal_conc():
    r1 = predict_cord_blood_level("Drug", mw_Da=300.0, logP=2.0,
                                   ionization_type="neutral",
                                   c_maternal_mg_L=1.0)
    r2 = predict_cord_blood_level("Drug", mw_Da=300.0, logP=2.0,
                                   ionization_type="neutral",
                                   c_maternal_mg_L=2.0)
...
def test_invalid_fu_zero_raises():
    with pytest.raises(ValueError, match="fu_maternal"):
        predict_cord_blood_level("X", mw_Da=300.0, logP=2.0, ionization_type="neutral",
                                  fu_maternal=0.0)
...
def test_screen_neonatal_exposure_sorted():
    drugs = [
        {"drug_name": "Small", "mw_Da": 200.0, "logP": 1.5, "ionization_type": "neutral",
         "fu_maternal": 0.9},
