...
def test_receptor_fractions_sum_le_one(base_result):
    for i, (ra, rd) in enumerate(
        zip(base_result.ra_fraction, base_result.rd_fraction)
    ):
        total = ra + rd
...
def test_longer_tau_less_tachyphylaxis(short_interval_result, long_interval_result):
    assert long_interval_result.tachyphylaxis_ratio >= short_interval_result.tachyphylaxis_ratio - 1e-6
