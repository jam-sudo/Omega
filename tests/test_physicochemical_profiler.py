...
class TestDevelopmentRisk:
    ...
    def test_low_risk_for_drug_like(self):
        ...
        result = _default(mw_Da=300.0, logp=2.0, hbd=0, hba=5, psa_A2=40.0, rotatable_bonds=5)
        assert result.development_risk == "low"

    def test_high_risk_for_poor_candidate(self):
        result = _default(mw_Da=700.0, logp=7.0, hbd=8, hba=15, psa_A2=200.0, rotatable_bonds=15)
...
class TestCompare:
    def test_compare_returns_sorted(self):
        candidates = [
            ...
            dict(
                drug_name="B", mw_Da=300.0, logp=2.0, hbd=0, hba=5, psa_A2=40.0, rotatable_bonds=5
            ),
            dict(
                drug_name="C", mw_Da=450.0, logp=3.0, hbd=2, hba=7, psa_A2=90.0, rotatable_bonds=8
            ),
        ]
    ...
    def test_compare_returns_list(self):
        candidates = [
            dict(
                drug_name="X", mw_Da=300.0, logp=2.0, hbd=1, hba=4, psa_A2=60.0, rotatable_bonds=4
            ),
        ]
