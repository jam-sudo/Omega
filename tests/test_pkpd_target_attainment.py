...
def make_result(
    pkpd_index="fT>MIC", target_value=40.0, mic_mg_L=0.125, dose_mg=500.0, dosing_interval_h=8.0
):
...
def test_auc_mic_low_mic_attained():
    ...
    result = make_result(pkpd_index="AUC/MIC", target_value=100.0, mic_mg_L=0.03, dose_mg=500.0)
    assert result.target_attained is True


def test_auc_mic_high_mic_not_attained():
    """High MIC → AUC/MIC target not attained."""
    result = make_result(pkpd_index="AUC/MIC", target_value=400.0, mic_mg_L=8.0, dose_mg=500.0)
    assert result.target_attained is False
...
def test_margin_positive_when_attained():
    ...
    result = make_result(pkpd_index="AUC/MIC", target_value=50.0, mic_mg_L=0.125, dose_mg=1000.0)
    assert result.target_attained is True
...
def test_margin_negative_when_not_attained():
    ...
    result = make_result(pkpd_index="AUC/MIC", target_value=10000.0, mic_mg_L=8.0, dose_mg=100.0)
    assert result.target_attained is False
    assert result.margin_pct < 0.0
...
def test_screen_custom_mics():
    custom = [0.5, 1.0, 2.0]
    results = screen_mic_range("TestDrug", 500.0, 8.0, 6.0, 30.0, mic_values=custom)
...
def test_screen_low_mic_attained_high_not():
    results = screen_mic_range(
        "TestDrug", 1000.0, 8.0, 6.0, 30.0, pkpd_index="AUC/MIC", target_value=100.0
    )
...
def test_invalid_pkpd_index():
    with pytest.raises(ValueError, match="pkpd_index"):
        assess_target_attainment("Drug", 500.0, 8.0, 6.0, 30.0, 0.5, pkpd_index="invalid")


def test_invalid_target_value():
    with pytest.raises(ValueError, match="target_value"):
        assess_target_attainment("Drug", 500.0, 8.0, 6.0, 30.0, 0.5, target_value=-10.0)
