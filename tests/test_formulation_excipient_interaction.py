...
def test_primary_amine_lactose_major_severity():
    result = predict_excipient_interaction(
        "PrimaryAmineDrug", "lactose", has_primary_amine=True
    )
...
def test_high_logp_hpmc_reduced_score():
    high_logp = predict_excipient_interaction(
        "LipophilicDrug", "HPMC", drug_logp=5.0
    )
    low_logp = predict_excipient_interaction(
        "HydrophilicDrug", "HPMC", drug_logp=1.0
    )
