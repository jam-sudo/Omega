...
def test_floating_retention_factor():
    ...
    assert result.retention_factor == pytest.approx(
        _FORMULATION_PARAMS["floating"]["retention_factor"]
    )
...
def test_bioadhesive_retention_factor():
    ...
    assert result.retention_factor == pytest.approx(
        _FORMULATION_PARAMS["bioadhesive"]["retention_factor"]
    )
...
def test_expandable_retention_factor():
    ...
    assert result.retention_factor == pytest.approx(
        _FORMULATION_PARAMS["expandable"]["retention_factor"]
    )
