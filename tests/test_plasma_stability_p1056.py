...
def test_rat_lower_t50_than_human_for_ester():
    ...
    human = predict_plasma_stability(
        "DrugE", mw_Da=400.0, logp=2.0, has_ester=True, species="human"
    )
...
def test_mouse_faster_than_rat_for_ester():
    ...
    rat = predict_plasma_stability("DrugE", mw_Da=400.0, logp=2.0, has_ester=True, species="rat")
    mouse = predict_plasma_stability(
        "DrugE", mw_Da=400.0, logp=2.0, has_ester=True, species="mouse"
    )
...
def test_peptide_bond_pathway():
    result = predict_plasma_stability("Peptide", mw_Da=800.0, logp=-1.0, has_peptide_bond=True)
