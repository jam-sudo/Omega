...
def test_abdomen_highest_f_among_sites():
    ...
    _ = [r.injection_site for r in results]
    assert results[0].injection_site == "abdomen"


def test_large_volume_feasibility():
    r = predict_sc_bioavailability(
        "Drug", mw_Da=400.0, logP=1.0, dose_mg=10.0, injection_volume_mL=5.0
    )
...
def test_feasible_volume():
    r = predict_sc_bioavailability(
        "Drug", mw_Da=400.0, logP=1.0, dose_mg=10.0, injection_volume_mL=1.0
    )
    assert r.dose_feasibility == "feasible"


def test_insoluble_drug_penalty():
    r_sol = predict_sc_bioavailability(
        "Drug", mw_Da=400.0, logP=1.0, dose_mg=10.0, solubility_mg_mL=100.0, injection_volume_mL=1.0
    )
    r_insol = predict_sc_bioavailability(
        "Drug", mw_Da=400.0, logP=1.0, dose_mg=100.0, solubility_mg_mL=0.05, injection_volume_mL=1.0
    )
...
def test_invalid_volume_raises():
    with pytest.raises(ValueError, match="injection_volume_mL"):
        predict_sc_bioavailability(
            "X", mw_Da=400.0, logP=1.0, dose_mg=10.0, injection_volume_mL=0.0
        )


def test_invalid_site_raises():
    with pytest.raises(ValueError, match="injection_site"):
        predict_sc_bioavailability("X", mw_Da=400.0, logP=1.0, dose_mg=10.0, injection_site="back")
