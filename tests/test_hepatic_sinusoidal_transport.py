...
class TestHepatocyteAccumulation:
    ...
    def test_higher_oatp_vmax_gives_higher_hepatocyte_auc(self):
        ...
        result_low = simulate_hepatic_sinusoidal_transport(
            "Drug", dose_mg=100.0, cl_sys_L_per_h=10.0, vd_L=50.0, oatp_vmax_uM_per_min=1.0
        )
        result_high = simulate_hepatic_sinusoidal_transport(
            "Drug", dose_mg=100.0, cl_sys_L_per_h=10.0, vd_L=50.0, oatp_vmax_uM_per_min=50.0
        )
    ...
    def test_higher_mrp2_gives_lower_hepatocyte_concentration(self):
        ...
        result_low_mrp2 = simulate_hepatic_sinusoidal_transport(
            "Drug", dose_mg=100.0, cl_sys_L_per_h=10.0, vd_L=50.0, mrp2_cl_mL_per_min=0.1
        )
        result_high_mrp2 = simulate_hepatic_sinusoidal_transport(
            "Drug", dose_mg=100.0, cl_sys_L_per_h=10.0, vd_L=50.0, mrp2_cl_mL_per_min=20.0
        )
        assert (
            result_low_mrp2.auc_hepatocyte_mg_h_per_L > result_high_mrp2.auc_hepatocyte_mg_h_per_L
        )
...
class TestOatpInhibitionComparison:
    def test_compare_returns_list(self):
        results = compare_oatp_inhibition("TestDrug", dose_mg=100.0, cl_sys_L_per_h=10.0, vd_L=50.0)
    ...
    def test_compare_custom_inhibition_factors(self):
        ...
        results = compare_oatp_inhibition(
            "TestDrug", dose_mg=100.0, cl_sys_L_per_h=10.0, vd_L=50.0, inhibition_factors=factors
        )
    ...
    def test_oatp_inhibition_reduces_hepatocyte_auc(self):
        ...
        results = compare_oatp_inhibition(
            "TestDrug", dose_mg=100.0, cl_sys_L_per_h=5.0, vd_L=50.0, inhibition_factors=[1.0, 0.01]
        )
