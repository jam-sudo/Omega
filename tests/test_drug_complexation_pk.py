...
def test_higher_cd_concentration_higher_enhancement():
    ...
    low_conc = predict_cyclodextrin_complexation("Drug", "HP-beta-CD", 2.0, cd_concentration_mM=5.0)
...
def test_invalid_cd_concentration_raises():
    with pytest.raises(ValueError, match="cd_concentration_mM"):
        predict_cyclodextrin_complexation("Drug", "HP-beta-CD", 2.0, cd_concentration_mM=0.0)


def test_negative_cd_concentration_raises():
    with pytest.raises(ValueError, match="cd_concentration_mM"):
        predict_cyclodextrin_complexation("Drug", "HP-beta-CD", 2.0, cd_concentration_mM=-5.0)


def test_invalid_drug_mw_raises():
    with pytest.raises(ValueError, match="drug_mw"):
        predict_cyclodextrin_complexation("Drug", "HP-beta-CD", 2.0, drug_mw=0.0)
