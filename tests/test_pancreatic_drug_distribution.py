...
class TestPlasmaConcentration:
    def test_plasma_starts_at_dose_over_vd(self):
        result = simulate_pancreatic_distribution(
            drug_name="Drug", dose_mg=200.0, vd_plasma_L=10.0
        )
...
class TestKpEffect:
    def test_higher_kp_higher_cmax_pancreas(self):
        r_low = simulate_pancreatic_distribution(
            drug_name="Drug", dose_mg=100.0, kp_pancreas=0.5
        )
        r_high = simulate_pancreatic_distribution(
            drug_name="Drug", dose_mg=100.0, kp_pancreas=3.0
        )
        assert r_high.cmax_pancreas_mg_L > r_low.cmax_pancreas_mg_L

    def test_kp_field_matches_input(self):
        result = simulate_pancreatic_distribution(
            drug_name="Drug", dose_mg=100.0, kp_pancreas=2.5
        )
