...
class TestBioequivalenceResultDataclass:
    def test_is_dataclass_with_required_fields(self):
        result = assess_bioequivalence(
            "DrugA", "DrugB",
            auc_test=_identical_auc(), auc_reference=_identical_auc(),
        )
    ...
    def test_has_n_subjects_field(self):
        result = assess_bioequivalence(
            "A", "B",
            auc_test=_identical_auc(10), auc_reference=_identical_auc(10),
        )
    ...
    def test_has_cmax_ratio_geometric_mean(self):
        result = assess_bioequivalence(
            "A", "B",
            auc_test=_identical_auc(), auc_reference=_identical_auc(),
            cmax_test=_identical_auc(), cmax_reference=_identical_auc(),
        )
...
class TestAssessBioequivalenceRatio:
    def test_ratio_one_for_identical_data(self):
        data = [80.0, 90.0, 100.0, 110.0, 120.0, 95.0,
                85.0, 105.0, 98.0, 102.0, 88.0, 112.0]
    ...
    def test_ratio_two_for_doubled_test(self):
        ref = [50.0, 60.0, 70.0, 80.0, 90.0, 100.0,
    ...
    def test_cmax_ratio_one_for_identical_cmax(self):
        ...
        result = assess_bioequivalence(
            "A", "B",
            auc_test=data, auc_reference=data,
            cmax_test=data, cmax_reference=data,
        )
...
class TestAssessBioequivalenceBEDecision:
    ...
    def test_overall_be_false_when_cmax_fails(self):
        ...
        result = assess_bioequivalence(
            "A", "B",
            auc_test=auc_ref, auc_reference=auc_ref,
            cmax_test=cmax_test, cmax_reference=cmax_ref,
        )
...
class TestAssessBioequivalenceCIOrdering:
    ...
    def test_ci_90_cmax_lower_less_than_upper(self):
        ...
        result = assess_bioequivalence(
            "A", "B",
            auc_test=test_data, auc_reference=ref_data,
            cmax_test=test_data, cmax_reference=ref_data,
        )
...
class TestSimulateBeStudy:
    ...
    def test_auc_ratio_close_to_true_ratio(self):
        ...
        result = simulate_be_study(
            "A", "B", n_subjects=200, true_ratio=1.1, intra_cv=0.2, seed=0
        )
