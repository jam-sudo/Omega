...
class TestReturnType:
    ...
    def test_field_names_present(self):
        result = _base_result()
        for field in [
            "drug_name", "polymer_type", "release_mechanism", "total_drug_mg",
            "particle_size_nm", "times_h", "cumulative_release_pct",
            "release_rate_pct_per_h", "t50_h", "t80_h", "burst_fraction_pct",
            "sustained_duration_h", "higuchi_fit_r2", "korsmeyer_n",
            "release_class", "notes",
        ]:
...
class TestKorsmeyerN:
    def test_n_in_valid_range(self):
        for polymer in _POLYMER_PARAMS:
            ...
            assert 0.0 < result.korsmeyer_n <= 1.0, (
                f"{polymer}: korsmeyer_n={result.korsmeyer_n}"
            )
