...
def test_return_type():
    result = predict_biorelevant_solubility("drug_a", pka=4.0, logP=2.0,
...
def test_fessif_greater_than_fassif_for_lipophilic():
    ...
    fassif = predict_biorelevant_solubility("lipophilic", pka=7.0, logP=3.0,
                                            ionization_type="neutral",
                                            intrinsic_solubility_mg_mL=0.05,
                                            medium="FaSSIF")
    fessif = predict_biorelevant_solubility("lipophilic", pka=7.0, logP=3.0,
                                            ionization_type="neutral",
                                            intrinsic_solubility_mg_mL=0.05,
                                            medium="FeSSIF")
...
def test_biorelevant_at_least_ionized_solubility():
    ...
    result = predict_biorelevant_solubility("acid_drug", pka=6.0, logP=2.0,
                                            ionization_type="acid",
...
def test_biorelevant_water_no_micellar():
    ...
    result = predict_biorelevant_solubility("neutral_drug", pka=7.0, logP=1.0,
                                            ionization_type="neutral",
...
def test_acid_higher_ph_increases_ionization_factor():
    ...
    fassif = predict_biorelevant_solubility("acid", pka=5.0, logP=1.0,
                                            ionization_type="acid",
                                            intrinsic_solubility_mg_mL=0.05,
                                            medium="FaSSIF")  # pH=6.5
    water = predict_biorelevant_solubility("acid", pka=5.0, logP=1.0,
                                           ionization_type="acid",
                                           intrinsic_solubility_mg_mL=0.05,
                                           medium="water")  # pH=7.0
...
def test_base_lower_ph_increases_ionization_factor():
    ...
    fassif = predict_biorelevant_solubility("base", pka=8.0, logP=1.0,
                                            ionization_type="base",
                                            intrinsic_solubility_mg_mL=0.05,
                                            medium="FaSSIF")  # pH=6.5
    fassgf = predict_biorelevant_solubility("base", pka=8.0, logP=1.0,
                                            ionization_type="base",
                                            intrinsic_solubility_mg_mL=0.05,
                                            medium="FaSSGF")  # pH=1.6
...
def test_neutral_ionization_factor_is_one():
    ...
    for medium in ["FaSSIF", "FeSSIF", "FaSSGF", "water", "PBS_pH6_8"]:
        result = predict_biorelevant_solubility("neutral", pka=7.0, logP=1.5,
                                                ionization_type="neutral",
                                                medium=medium)
...
def test_screen_media_food_effect_ratio_computed():
    ...
    results = screen_media("drug_x", pka=7.0, logP=3.0, ionization_type="neutral",
                           intrinsic_solubility_mg_mL=0.05)
...
def test_screen_media_fessif_over_fassif_ratio():
    ...
    expected_ratio = result_map["FeSSIF"].s_biorelevant_mg_mL / result_map["FaSSIF"].s_biorelevant_mg_mL
    for r in results:
...
def test_solubility_class_valid():
    for medium in VALID_MEDIA:
        result = predict_biorelevant_solubility("drug", pka=7.0, logP=1.0,
                                                ionization_type="neutral",
                                                medium=medium)
...
def test_solubility_class_high():
    result = predict_biorelevant_solubility("soluble_drug", pka=7.0, logP=-2.0,
                                            ionization_type="neutral",
...
def test_solubility_class_low():
    result = predict_biorelevant_solubility("insoluble_drug", pka=7.0, logP=5.0,
                                            ionization_type="neutral",
...
def test_s_biorelevant_always_positive():
    for medium in VALID_MEDIA:
        result = predict_biorelevant_solubility("drug", pka=4.0, logP=2.0,
                                                ionization_type="acid",
...
def test_base_drug_higher_solubility_in_fassgf_vs_water():
    ...
    fassgf = predict_biorelevant_solubility("base_drug", pka=9.0, logP=1.0,
                                            ionization_type="base",
                                            intrinsic_solubility_mg_mL=0.05,
                                            medium="FaSSGF")
    water = predict_biorelevant_solubility("base_drug", pka=9.0, logP=1.0,
                                           ionization_type="base",
                                           intrinsic_solubility_mg_mL=0.05,
                                           medium="water")
...
def test_invalid_medium_raises_value_error():
    with pytest.raises(ValueError, match="medium"):
        predict_biorelevant_solubility("drug", pka=7.0, logP=2.0,
                                       ionization_type="neutral",
...
def test_pka_out_of_range_raises():
    with pytest.raises(ValueError, match="pka"):
        predict_biorelevant_solubility("drug", pka=15.0, logP=2.0,
                                       ionization_type="neutral")


def test_logp_out_of_range_raises():
    with pytest.raises(ValueError, match="logP"):
        predict_biorelevant_solubility("drug", pka=7.0, logP=12.0,
                                       ionization_type="neutral")
...
def test_single_medium_food_effect_ratio_zero():
    ...
    result = predict_biorelevant_solubility("drug", pka=7.0, logP=2.0,
                                            ionization_type="neutral")
...
def test_pbs_ph6_8_no_micellar():
    ...
    result = predict_biorelevant_solubility("drug", pka=7.0, logP=3.0,
                                            ionization_type="neutral",
                                            medium="PBS_pH6_8")
    assert abs(result.micellar_enhancement - 1.0) < 1e-9
...
def test_intrinsic_solubility_stored_correctly():
    result = predict_biorelevant_solubility("drug", pka=7.0, logP=2.0,
                                            ionization_type="neutral",
                                            intrinsic_solubility_mg_mL=0.123)
    assert abs(result.s_intrinsic_mg_mL - 0.123) < 1e-10


def test_drug_name_stored():
    result = predict_biorelevant_solubility("ibuprofen", pka=4.4, logP=3.5,
                                            ionization_type="acid")
...
def test_acid_drug_lower_ionization_in_fassgf():
    ...
    fassgf = predict_biorelevant_solubility("acid_drug", pka=6.0, logP=2.0,
                                            ionization_type="acid",
                                            medium="FaSSGF")  # pH=1.6
    fassif = predict_biorelevant_solubility("acid_drug", pka=6.0, logP=2.0,
                                            ionization_type="acid",
                                            medium="FaSSIF")  # pH=6.5
