...
def default_neutral():
    return predict_salt_solubility(
        "NeutralDrug", pka=7.0, ionization_type="neutral", salt_form="free_base"
    )
...
def test_compare_salt_forms_sorted():
    ...
    results = compare_salt_forms(
        "Drug", pka=4.5, ionization_type="acid", logp=2.0, salt_forms=salt_forms
    )
