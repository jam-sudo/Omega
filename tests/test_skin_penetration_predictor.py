...
def test_screen_returns_sorted_descending_kp():
    candidates = [
        {"drug_name": "A", "mw_Da": 300.0, "logP": 1.0, "donor_conc_mg_mL": 1.0,
         "application_area_cm2": 10.0, "duration_h": 24.0},
        {"drug_name": "B", "mw_Da": 200.0, "logP": 3.0, "donor_conc_mg_mL": 1.0,
         "application_area_cm2": 10.0, "duration_h": 24.0},
        {"drug_name": "C", "mw_Da": 400.0, "logP": 0.5, "donor_conc_mg_mL": 1.0,
         "application_area_cm2": 10.0, "duration_h": 24.0},
    ]
...
def test_screen_preserves_count():
    candidates = [
        {"drug_name": f"Drug{i}", "mw_Da": 300.0, "logP": float(i), "donor_conc_mg_mL": 1.0,
         "application_area_cm2": 10.0, "duration_h": 24.0}
        for i in range(5)
    ]
    results = screen_transdermal_candidates(candidates)
...
def test_screen_single_candidate():
    candidates = [
        {"drug_name": "OnlyOne", "mw_Da": 300.0, "logP": 2.0, "donor_conc_mg_mL": 1.0,
         "application_area_cm2": 10.0, "duration_h": 24.0},
    ]
    results = screen_transdermal_candidates(candidates)
