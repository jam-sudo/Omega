...
def test_returns_excretion_route_result():
    result = predict_excretion_route("TestDrug", mw_Da=200.0, logP=0.5, ionization_type="neutral", fu_plasma=0.5)
...
def test_base_ionization_renal_bonus():
    result_base = predict_excretion_route("Drug", mw_Da=250.0, logP=0.5, ionization_type="base", fu_plasma=0.5)
    result_neutral = predict_excretion_route("Drug", mw_Da=250.0, logP=0.5, ionization_type="neutral", fu_plasma=0.5)
    assert result_base.renal_score == result_neutral.renal_score + 10.0


def test_acid_ionization_biliary_bonus():
    result_acid = predict_excretion_route("Drug", mw_Da=600.0, logP=4.0, ionization_type="acid", fu_plasma=0.5)
    result_neutral = predict_excretion_route("Drug", mw_Da=600.0, logP=4.0, ionization_type="neutral", fu_plasma=0.5)
...
def test_ckd_sensitivity_low_when_renal_score_low():
    result = predict_excretion_route("Drug", mw_Da=650.0, logP=4.5, ionization_type="acid", fu_plasma=0.05)
    # renal=0+0+0+0=0 → low
...
def test_screen_drugs_sorted_by_fe_renal_descending():
    drugs = [
        {"drug_name": "Biliary", "mw_Da": 650.0, "logP": 4.5, "ionization_type": "acid", "fu_plasma": 0.05},
        {"drug_name": "Renal", "mw_Da": 150.0, "logP": 0.2, "ionization_type": "base", "fu_plasma": 0.8},
        {"drug_name": "Mid", "mw_Da": 400.0, "logP": 2.0, "ionization_type": "neutral", "fu_plasma": 0.2},
    ]
    results = screen_drugs(drugs)
