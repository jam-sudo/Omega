...
def test_mechanism_valid():
    ...
    for tech in ["amorphous_dispersion", "cyclodextrin_complexation", "particle_size_reduction",
                 "salt_form", "cosolvency"]:
...
def test_salt_form_ionizable_higher_than_neutral():
    r_ion = predict_solubility_enhancement(
        drug_name="X", mw_Da=300.0, logp=2.0, melting_point_C=200.0,
        pka=4.0, ionization_type="acid", technique="salt_form"
    )
    r_neu = predict_solubility_enhancement(
        drug_name="X", mw_Da=300.0, logp=2.0, melting_point_C=200.0,
        pka=4.0, ionization_type="neutral", technique="salt_form"
    )
...
def test_cosolvency_higher_logp_higher_enhancement():
    r_high = predict_solubility_enhancement(
        drug_name="X", mw_Da=300.0, logp=5.0, melting_point_C=200.0,
        technique="cosolvency", cosolvent_pct=20.0
    )
    r_low = predict_solubility_enhancement(
        drug_name="X", mw_Da=300.0, logp=1.0, melting_point_C=200.0,
        technique="cosolvency", cosolvent_pct=20.0
    )
    assert r_high.enhancement_fold >= r_low.enhancement_fold
...
def test_validation_mw_zero():
    with pytest.raises(ValueError):
        predict_solubility_enhancement(
            drug_name="X", mw_Da=0.0, logp=2.0, melting_point_C=200.0
        )
...
def test_validation_cosolvent_out_of_range():
    with pytest.raises(ValueError):
        predict_solubility_enhancement(
            drug_name="X", mw_Da=300.0, logp=2.0, melting_point_C=200.0,
            technique="cosolvency", cosolvent_pct=150.0
        )
