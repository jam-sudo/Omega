...
class TestScreenTransporters:
    ...
    def test_returns_correct_count(self):
        results = screen_transporters("metformin", "cimetidine", self._ki_dict(), 5.0)
    ...
    def test_single_transporter_screen(self):
        results = screen_transporters("tenofovir", "probenecid", {"OAT1": 0.3}, 2.0)
    ...
    def test_kwargs_forwarded(self):
        results = screen_transporters(
            "drug_v", "drug_p", {"OCT2": 1.0}, 5.0, gfr_L_per_h=5.0, fu_plasma=0.3
        )
...
class TestValidation:
    ...
    def test_zero_gfr_raises_value_error(self):
        with pytest.raises(ValueError, match="gfr"):
            predict_renal_transporter_inhibition("a", "b", "OAT1", 1.0, 5.0, gfr_L_per_h=0.0)

    def test_fu_zero_raises_value_error(self):
        with pytest.raises(ValueError, match="fu_plasma"):
            predict_renal_transporter_inhibition("a", "b", "OAT1", 1.0, 5.0, fu_plasma=0.0)

    def test_fu_above_one_raises_value_error(self):
        with pytest.raises(ValueError, match="fu_plasma"):
            predict_renal_transporter_inhibition("a", "b", "OAT1", 1.0, 5.0, fu_plasma=1.1)
