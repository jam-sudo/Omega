...
from omega_pbpk.core.cerebrospinal_fluid_pk import CSFPKResult, simulate_csf_pk
...
def test_intrathecal_csf_starts_high():
    ...
    result = simulate_csf_pk("drug_a", dose_mg=500.0, logP=2.0, route="intrathecal")
    expected = 500.0 / 0.15
...
def test_meningitis_utility_false_when_ratio_below_01():
    result = simulate_csf_pk(
        "bad", dose_mg=500.0, logP=-2.0, route="iv", fu_plasma=0.01, cl_L_per_h=30.0, vd_L=50.0
    )
    if result.csf_to_plasma_auc_ratio <= 0.1:
