...
def test_initial_plasma_concentration():
    ...
    result = simulate_spleen_distribution(
        "DrugA", dose_mg=dose_mg, particle_size_nm=0, vd_sys_L=vd
    )
...
def test_spleen_has_peak():
    result = simulate_spleen_distribution(
        "DrugA", dose_mg=10.0, particle_size_nm=0, t_end_h=48.0
    )
...
def test_nanoparticle_mps_uptake_nonzero():
    ...
    result = simulate_spleen_distribution(
        "NanoDrug", dose_mg=10.0, particle_size_nm=100.0
    )
...
def test_higher_kp_spleen_increases_spleen_concentration():
    ...
    r_high = simulate_spleen_distribution(
        "Drug", dose_mg=10.0, particle_size_nm=0, kp_spleen=5.0
    )
