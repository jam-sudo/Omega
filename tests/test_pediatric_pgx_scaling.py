...
class TestReturnType:
    ...
    def test_all_fields_present(self, nm_child):
        fields = [
            "drug_name", "cyp_enzyme", "metabolizer_phenotype",
            "age_years", "weight_kg", "adult_dose_mg",
            "pediatric_dose_mg", "pgx_adjusted_dose_mg",
            "allometric_factor", "enzyme_maturation_factor",
            "pgx_dose_factor", "combined_dose_factor",
            "dose_cap_applied", "monitoring_required",
            "clinical_guidance", "notes",
        ]
        for f in fields:
...
class TestPMvsNM:
    ...
    def test_pm_clinical_guidance_mentions_toxicity(self, pm_child):
        assert "toxicity" in pm_child.clinical_guidance.lower() or "reduction" in pm_child.clinical_guidance.lower()
...
class TestUMvsNM:
    ...
    def test_um_clinical_guidance_mentions_efficacy(self, um_child):
        assert "efficac" in um_child.clinical_guidance.lower() or "higher" in um_child.clinical_guidance.lower()
...
class TestNeonate:
    def test_neonate_maturation_very_low(self, neonate_nm):
        ...
        assert neonate_nm.enzyme_maturation_factor < 0.05 or neonate_nm.enzyme_maturation_factor == pytest.approx(0.05, abs=0.01)
    ...
    def test_neonate_guidance_mentions_neonate(self, neonate_nm):
        assert "neonate" in neonate_nm.clinical_guidance.lower() or "neonat" in neonate_nm.clinical_guidance.lower()

    def test_neonate_dose_lower_than_older_child(self):
        neo = calculate_pediatric_pgx_dose(
            "D", "CYP2D6", "NM", 0.1, 3.5, 100.0, 0.8
        )
        older = calculate_pediatric_pgx_dose(
            "D", "CYP2D6", "NM", 10.0, 35.0, 100.0, 0.8
        )
...
class TestMaturationApproachesAdult:
    def test_maturation_near_1_for_teenager(self):
        result = calculate_pediatric_pgx_dose(
            "D", "CYP3A4", "NM", 15.0, 60.0, 100.0, 0.8
        )
...
class TestDoseCap:
    def test_dose_cap_applied_when_above_adult(self):
        ...
        result = calculate_pediatric_pgx_dose(
            "D", "CYP3A4", "UM", 17.0, 70.0, 100.0, 0.8
        )
    ...
    def test_dose_cap_not_applied_for_small_child(self, nm_child):
        ...
        assert not nm_child.dose_cap_applied or nm_child.pgx_adjusted_dose_mg <= nm_child.adult_dose_mg
...
class TestMonitoringRequired:
    ...
    def test_nm_older_child_may_not_need_monitoring(self):
        ...
        result = calculate_pediatric_pgx_dose(
            "D", "CYP3A4", "NM", 10.0, 35.0, 100.0, 0.5
        )
