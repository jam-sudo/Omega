...
The hybrid $C_{\max}$ selector is the dominant component: removing it (NO\_HYBRID) increases AAFE from 1.747 to 1.941 ($\Delta$+0.194 AAFE; 83\%$\to$58\% 2-fold). The bare ODE without any corrections achieves AAFE 2.039 ($\Delta$+0.292). Ridge residual correction contributes exactly zero ($\Delta$=0.000, confirmed dead code). (Table~\ref{tab:ablation}; Fig.~\ref{fig:ablation}).
...
NO\_PGP & 1.776 & [1.49, 2.17] & 79\% & +0.029 \\
NO\_VDSS & 1.783 & [1.48, 2.21] & 75\% & +0.036 \\
NO\_ENSEMBLE & 1.857 & [1.60, 2.20] & 71\% & +0.110 \\
NO\_ENS\_RIDGE & 1.857 & [1.60, 2.20] & 71\% & +0.110 \\
NO\_HYBRID & 1.941 & [1.65, 2.30] & 58\% & +0.194 \\
BARE & 2.039 & [1.72, 2.44] & 54\% & +0.292 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Ablation study: AAFE for 8 pipeline configurations. The hybrid $C_{\max}$ selector is the dominant component ($\Delta$+0.194 without it). Ridge correction contributes zero (NO\_RIDGE = FULL). Error bars indicate 95\% bootstrap CI.}
\label{fig:ablation}
\end{figure}
