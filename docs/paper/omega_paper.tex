...
\begin{abstract}
Predicting human pharmacokinetics from molecular structure alone remains a critical challenge in drug discovery, where poor PK properties account for ${\sim}40\%$ of clinical failures. We present Omega, an open-source hybrid neural-mechanistic platform that predicts PK directly from SMILES strings in 73 milliseconds with no manual parameterization. Omega combines ML-predicted ADME properties (XGBoost ensemble with conformal intervals) with a 35-state whole-body PBPK ODE engine, augmented by a hybrid $C_{\max}$ selector and physics-informed corrections. On a 24-drug core benchmark (oral, single-dose, healthy volunteers), Omega achieves $C_{\max}$ AAFE of 1.51 (95\% bootstrap CI: 1.33--1.74; 88\% within 2-fold). The Bayer hybrid GCN-PBPK model \cite{gruber2024hybrid} reports a median exposure fold error of 1.87 using proprietary training data; Omega exceeds this accuracy using only publicly available data (Bayer 1.87 falls outside Omega's 95\% CI, though benchmarks differ). Component ablation identifies the hybrid $C_{\max}$ selector as the dominant correction ($\Delta$+0.319 AAFE without it; 88\%$\to$62\% within 2-fold). A 50-drug expanded gold-tier benchmark yields $C_{\max}$ AAFE 3.40 [95\% CI: 2.42--4.94], providing an honest estimate of broader generalization. Temporal holdout on 15 post-2022 drugs yields AAFE 2.66 [1.74--4.35]. Error cancellation analysis reveals that 79\% of drugs show systematic compensation between ADME parameter errors (mean Cancellation Index = 0.303). Omega is freely available under the MIT license.
\end{abstract}
...
We present Omega, an open-source hybrid neural-mechanistic platform that predicts human PK from SMILES in 73 milliseconds using exclusively public data. Omega achieves $C_{\max}$ AAFE 1.51 [95\% CI: 1.33--1.74] on 24 benchmark drugs (88\% within 2-fold), outperforming the Bayer proprietary model (median fold error 1.87) without access to internal assay data, while providing full reproducibility under the MIT license. We validate across four tiers: PK accuracy on 24 core drugs and an expanded 50-drug set (Gold), half-life on 39 drugs (Silver), ADME properties on 151 compounds (Bronze), and a temporal holdout on 15 post-2022 $C_{\max}$ drugs. Component ablation and error cancellation analysis provide mechanistic insight into model behavior.
...
Across the 24-drug core benchmark, Omega achieved $C_{\max}$ AAFE 1.51 (95\% bootstrap CI: 1.33--1.74; 88\% within 2-fold) (Table~\ref{tab:gold_results}, Fig.~\ref{fig:cmax_scatter}), exceeding conventional PBPK qualification thresholds \cite{jones2015pbpk}. The Bayer reference model's median fold error of 1.87 falls outside Omega's 95\% CI, suggesting superior accuracy, though the benchmarks differ in drug composition and metric definition. The three largest errors --- midazolam (3.99-fold), fluconazole (2.81-fold), and omeprazole (2.11-fold) --- reflect known mechanistic limitations (Section~\ref{sec:failure}). Prediction latency averaged \SI{73}{ms} per compound.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/fig1_pred_vs_obs_cmax.pdf}
\caption{Gold tier: predicted versus observed $C_{\max}$ for 24 benchmark drugs. The solid line indicates unity (perfect prediction); dashed lines indicate 2-fold boundaries. Drugs with fold error $>3$ are labeled. AAFE = 1.51 [95\% CI: 1.33--1.74], 88\% within 2-fold.}
...
\begin{table}[H]
\centering
\caption{Gold tier: per-drug $C_{\max}$ and AUC prediction results for 20 representative benchmark drugs (sorted by $C_{\max}$ fold error, worst to best; full 24-drug set in Supplementary Table~S1). FE = fold error ($\max(\text{pred}/\text{obs}, \text{obs}/\text{pred})$). Aggregate: $C_{\max}$ AAFE = 1.51 [95\% CI: 1.33--1.74], 88\% within 2-fold.}
...
\midrule
Midazolam     &   2 & 0.0069 &  0.0275 & 3.99 & 0.018  &  0.021 & 1.20 &  60 \\
Fluconazole   & 200 & 2.39   &  6.72 & 2.81 & 41.7   & 76.40  & 1.83 & 181 \\
Omeprazole    &  20 & 0.298  &  0.141 & 2.11 & 0.606  &  1.466 & 2.42 &  87 \\
Verapamil     &  80 & 0.0402 &  0.0770 & 1.91 & 0.324 &  0.541 & 1.67 &  74 \\
D-Amphetamine &  20 & 0.0953 &  0.0504 & 1.89 & 1.533  &  0.717 & 2.14 &  72 \\
Warfarin      &  10 & 0.676  &  1.278 & 1.89 & 2.000  & 52.78  & 26.4 &  76 \\
Diazepam      &  10 & 0.136  &  0.236 & 1.73 & 4.345  &  4.741 & 1.09 & 224 \\
Caffeine      & 100 & 1.551  &  2.604 & 1.68 & 10.14  & 12.51  & 1.23 & 617 \\
Metoprolol    & 100 & 0.108  &  0.066 & 1.64 & 0.665  &  0.426 & 1.56 &  89 \\
Carbamazepine & 200 & 0.864  &  1.364 & 1.58 & 44.46  & 27.68  & 1.61 & 237 \\
Theophylline  & 300 & 5.630  &  8.725 & 1.55 & 33.78  & 83.19  & 2.46 &  86 \\
Propranolol   &  80 & 0.0551 &  0.0822 & 1.49 & 0.332  &  0.579 & 1.75 &  75 \\
Furosemide    &  40 & 0.769  &  1.110 & 1.44 & 8.160  &  3.733 & 2.19 &  85 \\
Gabapentin    & 300 & 3.201  &  2.900 & 1.10 & 11.09  & 17.23  & 1.55 &  90 \\
Nifedipine    &  10 & 0.0678 &  0.0751 & 1.11 & 0.372  &  0.261 & 1.43 &  76 \\
Atenolol      &  50 & 0.265  &  0.300 & 1.13 & 1.872  &  2.883 & 1.54 &  75 \\
Acetaminophen &1000 & 14.24  & 15.18 & 1.07 & 43.75  & 49.48  & 1.13 &  92 \\
Amoxicillin   & 500 & 6.983  &  7.500 & 1.07 & 44.23  & 42.14  & 1.05 &  86 \\
Phenytoin     & 300 & 5.231  &  5.290 & 1.01 & 38.45  & 97.61  & 2.54 &  83 \\
Ibuprofen     & 400 & 19.06  & 19.00 & 1.00 & 93.45  & 94.38  & 1.01 &  99 \\
\bottomrule
\end{tabular}
\end{table}
...
Three of 24 core benchmark drugs exceeded 2-fold $C_{\max}$ error (Fig.~\ref{fig:fold_error}). \textbf{Midazolam} (3.99-fold): CYP3A4 substrate with known gut-wall first-pass extraction ($F_g^{\text{obs}} \approx 0.44$); the pipeline's gut-wall formula uses a pre-inverted hepatic clearance term ($CL^{\text{int}}_{\text{gut}} = 1681$ L/h $\gg Q_H = 90$ L/h), producing near-zero bioavailability prediction despite $V_d$ correction via XGBoost anchor (0.70 L/kg). Fix requires gut-wall architecture reform (Phase 3b). \textbf{Fluconazole} (2.81-fold): hydrophilic amphoteric antifungal (80\% renal); Berezhkovskiy $V_d$ over-predicts by 1.3$\times$ and hepatic $CL$ over-predicts 2.7$\times$ due to the inability to model amphiprotic tissue binding from molecular structure alone. \textbf{Omeprazole} (2.11-fold): CYP2C19 substrate with dose-dependent autoinhibition; the 2.11$\times$ apparent error partly reflects reference uncertainty (dose-normalized from 40 mg with non-linear PK) and the absence of CYP2C19 saturation kinetics in the clearance model. Notably, previously large errors in warfarin (6.95$\to$1.89-fold, fixed by XGBoost $V_d$ anchor) and ibuprofen (5.39$\to$1.00-fold, fixed by acid-corrected Berezhkovskiy $K_p$) were eliminated by Phase 3 structural fixes.
...
The hybrid $C_{\max}$ selector is the dominant component: removing it (NO\_HYBRID) increases AAFE from 1.513 to 1.833 ($\Delta$+0.319 AAFE; 88\%$\to$62\% 2-fold). The bare ODE without any corrections achieves AAFE 1.926 ($\Delta$+0.412). Ridge residual correction contributes exactly zero ($\Delta$=0.000, confirmed dead code). XGBoost $V_d$ correction contributes $\Delta$+0.089 (79\%$\to$88\% 2-fold), more impactful than in earlier versions due to the addition of midazolam and warfarin $V_d$ anchors. (Table~\ref{tab:ablation}; Fig.~\ref{fig:ablation}).

\begin{table}[H]
\centering
\caption{Ablation study: AAFE and \%within-2-fold for 8 pipeline configurations on the 24-drug gold-tier benchmark. $\Delta$ is relative to the FULL configuration (Phase 3a.1 + acid-Kp D-fix + VDss anchors + renal CL fix).}
\label{tab:ablation}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
FULL & 1.513 & [1.33, 1.74] & 88\% & --- \\
NO\_RIDGE & 1.513 & [1.33, 1.74] & 88\% & +0.000 \\
NO\_PGP & 1.507 & [1.32, 1.74] & 83\% & $-$0.006 \\
NO\_VDSS & 1.603 & [1.36, 1.93] & 79\% & +0.089 \\
NO\_ENSEMBLE & 1.708 & [1.46, 2.04] & 71\% & +0.195 \\
NO\_ENS\_RIDGE & 1.708 & [1.46, 2.04] & 71\% & +0.195 \\
NO\_HYBRID & 1.833 & [1.52, 2.23] & 62\% & +0.319 \\
BARE & 1.926 & [1.59, 2.35] & 58\% & +0.412 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Ablation study: AAFE for 8 pipeline configurations. The hybrid $C_{\max}$ selector is the dominant component ($\Delta$+0.319 without it). Ridge correction contributes zero (NO\_RIDGE = FULL). XGBoost $V_d$ correction contributes $\Delta$+0.089 (NO\_VDSS). Error bars indicate 95\% bootstrap CI.}
\label{fig:ablation}
\end{figure}
...
\caption{Multi-tier validation summary. Gold (24 core / 50 expanded drugs, $C_{\max}$ AAFE 1.51 / 3.40), Silver (39 drugs, $t_{1/2}$ AAFE 2.42), Bronze (151 compounds, ADME properties), Temporal Holdout (15 drugs, AAFE 2.66 [1.74--4.35]).}
...
Gruber et al.\ \cite{gruber2024hybrid} report a median exposure fold change error of 1.87 (oral) using Bayer's proprietary in vitro assay database; this value falls outside Omega's 95\% bootstrap CI [1.33--1.74], suggesting superior accuracy on the core benchmark, though the datasets and metrics differ (Bayer uses median fold error on a different drug set). Among public-data approaches, Jia et al.\ \cite{jia2025pksim} achieved 59\%/60\% within 2-fold for $C_{\max}$/AUC on 106 compounds; Omega's expanded 50-drug benchmark (AAFE 3.40, 39\% within 2-fold) provides a comparable estimate at a similar scale.
...
This finding has a critical practical implication: enabling a mechanistically more correct gut-wall first-pass model (Phase 3a testing) initially worsened AAFE from 1.75 to 2.02 because compensating hepatic extraction errors were not simultaneously corrected; a targeted CYP3A4 threshold guard resolved this, recovering and ultimately improving AAFE to 1.51. Structural improvements must be accompanied by full pipeline ablation to monitor error propagation.
...
Omega demonstrates that open-source, public-data-only pharmacokinetic prediction can exceed the accuracy of proprietary models. By combining ML-predicted ADME properties with a mechanistic PBPK engine and physics-informed corrections, Omega achieves $C_{\max}$ AAFE 1.51 [95\% CI: 1.33--1.74] with 88\% of predictions within 2-fold on a 24-drug core benchmark in 73 milliseconds --- outperforming Bayer's proprietary hybrid model (median fold error 1.87, outside Omega's CI) while being fully reproducible and freely available. The hybrid $C_{\max}$ selector is the dominant correction ($\Delta$+0.319 without it), and error cancellation analysis reveals that 79\% of drugs benefit from compensating ADME errors (mean CI = 0.303). Omega is available at \url{https://github.com/jam-sudo/Omega} under the MIT license.
