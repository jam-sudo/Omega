...
def _validate_inputs(victim_fup_baseline, ki_displacement_uM, displacer_conc_uM,
                     victim_total_bound_fraction, binding_protein):
    if not (0 < victim_fup_baseline <= 1.0):
        raise ValueError(
            f"victim_fup_baseline must be in (0, 1], got {victim_fup_baseline}"
        )
    if ki_displacement_uM <= 0:
        raise ValueError(
            f"ki_displacement_uM must be > 0, got {ki_displacement_uM}"
        )
    if displacer_conc_uM < 0:
        raise ValueError(
            f"displacer_conc_uM must be >= 0, got {displacer_conc_uM}"
        )
...
def _build_notes(victim_drug, displacer_drug, binding_protein, fup_increase_fold,
                 clinical_significance, monitoring_required, narrow_ti):
    lines = [
    ...
    if monitoring_required:
        lines.append(
            "Therapeutic drug monitoring and clinical observation are recommended."
        )
...
def predict_displacement_interaction(
    ...
    notes = _build_notes(
        victim_drug, displacer_drug, binding_protein,
        fup_increase_fold, clinical_significance, monitoring_required, narrow_ti
    )
...
def screen_displacers(victim_drug, victim_fup_baseline, victim_total_bound_fraction, displacers_list):
