...
def _validate_inputs(
    ...
    if cyp_enzyme not in valid_cyps:
        raise ValueError(
            f"cyp_enzyme must be one of {valid_cyps}, got '{cyp_enzyme}'"
        )
    if not (0.0 <= fraction_metabolized_by_cyp <= 1.0):
        raise ValueError(
            f"fraction_metabolized_by_cyp must be in [0, 1], "
            f"got {fraction_metabolized_by_cyp}"
        )
...
def calculate_pediatric_pgx_dose(
    ...
    _validate_inputs(
        ...
        age_years, weight_kg, adult_dose_mg, fraction_metabolized_by_cyp,
    )
