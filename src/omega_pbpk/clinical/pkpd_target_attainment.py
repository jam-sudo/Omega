...
def _validate_inputs(
    dose_mg, cl_L_per_h, vd_L, mic_mg_L, pkpd_index, target_value, dosing_interval_h
):
    if dose_mg <= 0:
...
def _compute_f_time_above_mic(
    dose_mg, vd_L, ka_per_h, ke, f_oral, mic_mg_L, dosing_interval_h, dt=0.01
):
    """
...
def _find_breakpoint_ft_mic(
    dose_mg, vd_L, ka_per_h, ke, f_oral, dosing_interval_h, target_pct, dt=0.01
):
    """
    ...
    ft_lo = _compute_f_time_above_mic(
        dose_mg, vd_L, ka_per_h, ke, f_oral, lo, dosing_interval_h, dt
    )
    if ft_lo < target_pct:
        # Even at very low MIC, target not met
        return 0.0

    ft_hi = _compute_f_time_above_mic(
        dose_mg, vd_L, ka_per_h, ke, f_oral, hi, dosing_interval_h, dt
    )
    if ft_hi >= target_pct:
        return hi
    ...
    for _ in range(50):
        mid = (lo + hi) / 2.0
        ft_mid = _compute_f_time_above_mic(
            dose_mg, vd_L, ka_per_h, ke, f_oral, mid, dosing_interval_h, dt
        )
...
def assess_target_attainment(
    ...
    f_time_pct = _compute_f_time_above_mic(
        dose_mg, vd_L, ka_per_h, ke, f_oral, mic_mg_L, dosing_interval_h
    )
    ...
    if pkpd_index == "AUC/MIC":
        ...
        breakpoint = _find_breakpoint_ft_mic(
            dose_mg, vd_L, ka_per_h, ke, f_oral, dosing_interval_h, target_value
        )
