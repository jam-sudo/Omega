...
def predict_excipient_interaction(
    ...
    if excipient not in _EXCIPIENTS:
        raise ValueError(f"Unknown excipient '{excipient}'. Valid options: {sorted(_EXCIPIENTS)}")
    if drug_charge not in _VALID_CHARGES:
        raise ValueError(f"drug_charge must be one of {_VALID_CHARGES}, got '{drug_charge}'")
