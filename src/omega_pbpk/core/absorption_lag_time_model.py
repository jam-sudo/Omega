...
def _validate_inputs(dose_mg, t_lag_h, ka_per_h, cl_L_per_h, vd_L, f_oral,
                     t_end_h, dt_h):
    if dose_mg <= 0:
...
def _simulate_ode(dose_mg, t_lag_h, ka_per_h, cl_L_per_h, vd_L, f_oral,
                  t_end_h, dt_h):
    """
...
def _trapezoidal_auc(x, y):
    ...
    return sum(0.5 * (y[i] + y[i - 1]) * (x[i] - x[i - 1])
...
def simulate_absorption_lag(
    ...
    AbsorptionLagResult
    """
    _validate_inputs(dose_mg, t_lag_h, ka_per_h, cl_L_per_h, vd_L, f_oral,
                     t_end_h, dt_h)
