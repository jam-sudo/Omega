...
def main():
    ...
    for drug_name, info in BENCHMARK_DRUGS.items():
        ...
        try:
            ...
            print(f"  ADME: mw={adme.mw:.1f}, logP={adme.logP:.2f}, fup={adme.fup:.4f}, "
            ...
            print(f"  IVIVE: clint_hepatocyte={clint_hep_raw:.3f}, clint_L_per_h={clint_L_per_h:.4f} ({ivive_source})")
            ...
            print(f"  Predicted: Cmax={pred_cmax:.4f} mg/L, AUC={pred_auc:.4f} mg*h/L, "
            ...
            entry = {
                ...
                "adme": {
                    "mw": adme.mw, "logP": adme.logP, "fup": adme.fup,
                    "rbp": adme.rbp, "clint_3a4": adme.clint_3a4, "peff": adme.peff,
                },
                "success": True,
            }
    ...
    print(f"AUC:  AAFE={aafe_auc:.2f}, within 2-fold={pct_2fold_auc:.0f}% ({sum(1 for fe in valid_auc if fe <= 2.0)}/{len(valid_auc)})")

    print(f"\n--- EXIT CRITERIA CHECK ---")
    print(f"  AAFE < 3.0:  Cmax={'PASS' if aafe_cmax < 3.0 else 'FAIL'} ({aafe_cmax:.2f}), AUC={'PASS' if aafe_auc < 3.0 else 'FAIL'} ({aafe_auc:.2f})")
    print(f"  ≤2-fold ≥70%: Cmax={'PASS' if pct_2fold_cmax >= 70 else 'FAIL'} ({pct_2fold_cmax:.0f}%), AUC={'PASS' if pct_2fold_auc >= 70 else 'FAIL'} ({pct_2fold_auc:.0f}%)")
